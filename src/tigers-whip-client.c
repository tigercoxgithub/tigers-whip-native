...
int main(int argc, char *argv[]) {
    ...
    WHIP_LOG(LOG_INFO, "Tiger added log level: %i\n\n", whip_log_level );
	...
	gst_init(NULL, NULL);
	/* Make sure our gstreamer dependency has all we need Tiger could edit this if it changes*/
...
static gboolean whip_check_plugins(void) {
	...
	gboolean ret = TRUE;
	 ...
	 printf("Tigers test x5... \n %s \n",  resource_url);
     for(int i = 0; i < 5; i++){
    printf("How do I search gstreamer plugins here:  %s \n", resource_url );
}
...
static void whip_connect(GstWebRTCSessionDescription *offer) {
	...
	g_free(sdp_offer);
	if(status != 201) { //was 202 tigers thinks
		/* Didn't get the success we were expecting */
		WHIP_LOG(LOG_ERR, " [%u] %s\n", status, status ? session.msg->reason_phrase : "HTTP error");
		/* Get the response */
	   const char *tiger_body = session.msg->response_body->data;
      printf("Tiger added cloudflare resp:  %s \n", tiger_body );
		g_object_unref(session.msg);
		g_object_unref(session.http_conn);
